...
function upload($DATA_URL, $USERNAME, $PASSWORD, $UPDATE_FACILITY_MODE, $UPDATE_COMMODITY_NAMES_MODE, $UPDATE_COMMODITY_DATA_MODE, $COMMODITY_NAMES_IDS_FILE, $db) {
	...
	$data_json = file_get_contents ( 'Commodities_analytics_Jan_Jun_2014.json' ); // REMOVE: for test only
	...
	$date = "201401";
      ...
      // echo '<br/>The size of the whole file from Jan to Dec, 2014 is '.$count.'<br/><br/>';
		...
		if($faci_exists_count<=0){
			...
			if($db_facility_info && array_key_exists($facility_external_id, $db_facility_info)){
				...
				$consumption = $commodity[3];
                                ...
                                $STOCK_OUT_INDICATORS = "JyiR2cQ6DZT";
$STOCK_OUT_IMPLANT = "wNT8GGBpXKL";
                                $stock_outs = (($uuid==$STOCK_OUT_INDICATORS || $uuid==$STOCK_OUT_IMPLANT) && ($consumption=="1.0" || $consumption=="1" || $consumption=="100" || $consumption=="100.0"))?"Y":"N";
                                $bringer = 0;
				try{
...
// add new data
                                          echo $consumption.' iuud'.$uuid.' stock out indicator'.$STOCK_OUT_INDICATORS.' implant stock out '.$STOCK_OUT_IMPLANT.'<br/>';
					$bind = array(
