...
$consump_loc = array();
     ...
     //print_r($locations);
                ...
                $stock_out = $this->getSanParam('stock_out');
                $start_day = $reports->check_length_add_one($this->getSanParam('start-day'));
                $start_month = $reports->check_length_add_one($this->getSanParam('start-month'));
                $start_year = $this->getSanParam('start-year');
               // echo $start_day;
                
                
                $end_day = $reports->check_length_add_one($this->getSanParam('end-day'));
                $end_month = $reports->check_length_add_one($this->getSanParam('end-month'));
                $end_year = $this->getSanParam('end-year');
                
               $dob_start_day = $reports->check_length_add_one($this->getSanParam('dob-start-day'));
                $dob_start_month = $reports->check_length_add_one($this->getSanParam('dob-start-month'));
                $dob_start_year = $this->getSanParam('dob-start-year');
                
                $dob_end_day = $reports->check_length_add_one($this->getSanParam('dob-end-day'));
                $dob_end_month = $reports->check_length_add_one($this->getSanParam('dob-end-month'));
             ...
             $location_name = $this->get_location_name($location_id);
              foreach($training_type as $trainingtype){
                  ...
                  foreach($trainingorganizer as $training_org){
					  ...
					  $trainingorgq = "AND training_organizer_option_id=".$training_org."";
                      $data_collector = array();
                      $facility_ids = array();
    ...
    $error[] = "For this report option, Location is a required field";
         ...
         $locations = $this->get_all_locations();
              ...
              if($cadre==""){
                  ...
                  $cadrequery = "AND facility_summary_person_view.primary_qualification_option_id='".$cadre."'";
              }
              ...
              if($training_organizer==""){
                  ...
                  $training_organizer = implode(",",$trainingorganizer);
                  $trainingorganizerquery = "AND facility_summary_person_view.training_organizer_option_id IN (".$training_organizer.")";
              }
              
              if($end_day=="" && $end_month=="" && $end_year==""){
                  $training_end_date  = "";
              }else{
                  $training_end_date = "AND facility_summary_person_view.training_start_date>='".$start_date."' AND facility_summary_person_view.training_end_date<='".$end_date."'";
              }
             if(!empty($facility)){
                  $facer = implode(",",$facility);
                  $facilityq = "AND facility_summary_person_view.facility_id IN (".$facer.")";
                  }
              else{
                  $facilityq = "";
              }
             ...
             // echo $facer;exit;
              if($trainingtypes==""){
                  ...
                  $trainingtypequery = "AND facility_summary_person_view.training_title_option_id IN (".$trainingtypes.")";
              }
              if($certification==""){
                  $certificationq = "";
              }else{
                  $certificationq = " AND facility_summary_person_view.certification='".$certification."'";
              }
              if($gender==""){
                  $genderq = "";
              }
...
$criteria['start_day'] = ($this->getSanParam('start-day')!="")?$reports->check_length_add_one($this->getSanParam('start-day')):$start_day;
$criteria['start_month'] = ($this->getSanParam('start-month')!="")?$reports->check_length_add_one($this->getSanParam('start-month')):$start_month;
 $criteria['start_year'] = ($this->getSanParam('start-year')!="")?$this->getSanParam('start-year'):$start_year;
               // echo $start_day;
             
                
 $criteria['end_day'] = ($this->getSanParam('end-day')!="")?$reports->check_length_add_one($this->getSanParam('end-day')):$end_day;
$criteria['end_month'] = ($this->getSanParam('end-month')!="")?$reports->check_length_add_one($this->getSanParam('end-month')):$end_month;
...
('num_location_tiers'));
		$this->viewAssignEscaped ( 'tlocations', $tlocations );
               ...
               foreach($facilities_new as $facesss){
                ...
                $search_type = $this->getSanParam('cumu');
                $stock_out = $this->getSanParam('stock_out');
...
$consump_loc = array();
  ...
  //print_r($locations); exit;
	  ...
	  $error[] = "You need to select the period";
         ...
         $error[] = "Geography is required (at least state)";
         ...
         else if($agrregate_method=="agrregate_data" && $training_type==""){
             ...
             if($agrregate_method=="agrregate_data"){
                 ...
                 if($stock_out!=""){
				  ...
				  //echo 'Hello';exit;
				  $facilityQuery = "AND facility_id IN (".$facilitators.")";
  ...
  // echo $sql;exit;
   ...
   $res = $db->fetchAll($sql);
      ...
      $res = $db->fetchAll($sql);
       ...
       public function get_all_participants_with_all_conditions($locator,$cadrequery,$trainingorganizerquery,$trainingtypequery,$training_end_date,$genderq,$certificationq,$facilityq="",$dob_query=""){
           $db = Zend_Db_Table_Abstract::getDefaultAdapter ();
       $sql = "SELECT facility_summary_person_view.*,person.birthdate FROM facility_summary_person_view left join person on person.id=person_id WHERE facility_location_id IN (".$locator.") OR parent_id IN (".$locator.") ".$cadrequery." ".$trainingorganizerquery." ".$trainingtypequery." ".$training_end_date." ".$genderq." ".$certificationq." ".$facilityq." ".$dob_query;    
 //echo $sql;exit;
       $res = $db->fetchAll($sql);
