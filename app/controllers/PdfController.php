         ...
         if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            //echo 'before'; exit;
                ...
                *  $tierValue and $location_id respectively.
                 */
                $reportLocation = $pdf->getNextLocationDetailsWithTiers(3,"1978");
...
mkdir("pdfrepo/lga/".$folder_name."");
      ...
      // echo $folder_name;exit;
        ...
        $percent_fp_trained_not_providing_for_this_state_value = round($fp_trained_not_providing[0] * 100,2);
      
        $facilities_fptrained_not_providing = $pdf->fetchFacilitiesWithHWNotProviding("fp","fp",$location_id,3);
        //print_r($facilities_fptrained_not_providing);exit;
        ...
        $percent_larc_trained_not_providing_for_this_state_value = round($larc_trained_not_providing[0] * 100,2);
       
        
        $facilities_larctrained_not_providing = $pdf->fetchFacilitiesWithHWNotProviding("larc","larc",$location_id,3);
        //print_r($facilities_larctrained_not_providing);exit;
        ...
        $state_stocked_out_fp = $stockout->fetchPercentFacsProvidingButStockedOut("fp",$parent_id,2);
       // print_r($state_stocked_out_fp);exit;
        $state_stock_out_fp_seven_days = round($state_stocked_out_fp[1]['percent']*100,2);
        $lga_stock_out_seven_days = $stockout->fetchPercentFacsProvidingButStockedOut("fp",$location_id,3);
        $lga_stocked_out_fp_seven_days = round($lga_stock_out_seven_days[1]['percent']*100,2);
        ...
        $state_stocked_out_larc = $stockout->fetchPercentFacsProvidingButStockedOut("larc",$parent_id,2);
        $state_stock_out_larc = round($state_stocked_out_larc[1]['percent']*100,2);
    ...
    public function statechartsAction(){
        ...
        //exit;
           //$pdf->update_database("2015-01-01", "2015-05-01");
        if ($this->getRequest()->isXmlHttpRequest()) {
            if ($this->getRequest()->isPost()) {
                ...
                *  $tierValue and $location_id respectively.
                 */
                $reportLocation = $pdf->getNextLocationDetailsWithTiers(2,"970");
 ...
 //  echo $local_list;
    // echo 'working';exit;
     ...
     // fetchFacsWithHWProviding
      ...
      for($i=0;$i<$sizeof;$i++){
       ...
       $stock_out_fp_seven_days = $stockout->fetchPercentFacsProvidingButStockedOut("fp", $local_list,3);
       $state_stocked_out = $stockout->fetchPercentFacsProvidingButStockedOut("fp",$location_id,2);
       ...
       $stock_out_larc_implants = $stockout->fetchPercentFacsProvidingButStockedOut("larc", $local_list,3);
       $state_stocked_out_implants = $stockout->fetchPercentFacsProvidingButStockedOut("larc",$location_id,2);
       ...
       $stock_out_larc_implants[0]['percent'] = $state_stocked_out_implants[1]['percent'];
           ...
           for($i=1;$i<6;$i++){
               ...
               //echo 'hello';
               $percent_fp_trained = round($fp_trained_lgas[$i]['percent']*100,2);
               $percent_larc_trained = round($larc_trained_lgas[$i]['percent']*100,2);
               
               $fptrained = $fp_trained_lgas[$i]['location']." (".$percent_fp_trained."%)";
               $larctrained = $larc_trained_lgas[$i]['location']." (".$percent_larc_trained."%)";
              
               if($fp_trained_lgas[$i]['percent']==0)$fp_trained_lgas[$i]['percent']=0;                   
               if($larc_trained_lgas[$i]['percent']==0)$fp_trained_lgas[$i]['percent']=0;  
               
               $percent_fp_prov = round($fp_trained_lgas[$i]['percent']*100,2);
               $percent_larc_prov = round($larc_trained_lgas[$i]['percent']*100,2);
   ...
   //print_r($fp_array);exit;
       ...
       //TP: assigning the datas to the placeholder to be used in the view
        ...
        $this->view->assign('larc_percent_providing',$larc_percent_multiple);
        $this->view->assign('fp_percent_providing',$fp_percent_multiple);
    ...
    public function showreportsAction(){     
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            if ($this->getRequest()->isPost()) {
                ...
                *  $tierValue and $location_id respectively.
                 */
                $reportLocation = $pdf->getNextLocationDetailsWithTiers("","0");
...
//echo $reportId;exit;
        ...
        $fp_percent_per_state = $coverage->fetchPercentFacHWTrainedPerLocation('fp');
        ...
        $larc_percent_per_state = $coverage->fetchPercentFacHWTrainedPerLocation('larc');
        $percentData = array();
        ...
        $fp_providing_per_state = $coverage->fetchPercentFacsProvidingPerLocation('fp');
        $percentData = array();
        ...
        $larc_providing_per_state = $coverage->fetchPercentFacsProvidingPerLocation('larc');
        $percentData = array();
        ...
        $larc_stockout_per_state = $coverage->fetchFacsWithHWProviding("larc","larc",'',2);  //$stockout->fetchPercentStockOutFacsWithTrainedHWPerStates('larc');
...
//TP: Generating the table for the FP trained hw and not providing FP
   ...
   $table_structure =  '<br/><br/><div class="row" style="top:120;">
                    <fieldset class="">
...
//TP: Generating the table for the LARC trained hw and not providing LARC
   ...
   $table_structure .=  ''. $br.'<br/><br/><br/><br/><br/><br/><br/><div class="row" style="top:120;">
                    <fieldset class="">
...
//TP: Generating the table for the LARC implant stock out
   ...
   $table_structure .=  ''. $br.'<br/><br/><br/><br/><br/><br/><div class="row" style="top:120;">
                    <fieldset class="">
    ...
    public function pdfMailingListAction(){
		...
		$this->_countrySettings = System::getAll();
               ...
               foreach($allUserDetails as $userDetails){
                   ...
                   //Tp: this is where the emails for the test users is inserted into an array.
                   $emailme = array("bjaypjay2012@gmail.com");
                   
                   if(!in_array($email,$emailme)){
                       continue;
                   }
                   //$email = "obigregory@techieplanetltd.com";
                   //$email = "leke@techieplanetltd.com";
                   if($role==1){
                       //admin mailing lists
                   }
                   else if($role==2 || $role==3){
                       //fmoh and partner mails
                     ...
                     $result = $this->sendMail($fileAttachmentPaths,$email,$month,$year,$firstName,$lastName,"fmohandpartner");
if($result){
    echo 'Pdf Report Sent to user <b>'.$firstName.' '.$lastName.'</b> with username <b>'.$username.'</b><br/>';
} else{
    echo 'Pdf Report Not Sent to user <b>'.$firstName.' '.$lastName.'<b> with username <b>'.$username.'</b><br/>';
}                    
//print_r($fileAttachmentPaths);
    ...
    echo 'Pdf Report Not Sent to user <b>'.$firstName.' '.$lastName.'<b> with username <b>'.$username.'</b><br/>';
                       ...
                       //lga mails
                       $region_c_ids[] = $region_c_id;
                        if(!empty($region_c_id) || $region_c_id!=0){
...
<br/><br/>";
        ...
        $html.="Forgot your password? You can retrieve it by navigating to the log-in page of the Dashboard, clicking the Forgot Password?link, and following the instructions. For further assistance with the FP Dashboard, please contact: <a href='mailto:support@fpdashboard.ng'>support@fpdashboard.ng</a>.";
...
if(!isset($locationsArray) || empty($locationsArray)){
    $error[] = "The least Criterion to be selected is State";
}else {
...
foreach($achivedReports as $archived){
 ...
 $criteria['start_day'] = ($this->getSanParam('start-day')!="")?$report->check_length_add_one($this->getSanParam('start-day')):$startDay;
$criteria['start_month'] = ($this->getSanParam('start-month')!="")?$report->check_length_add_one($this->getSanParam('start-month')):$startMonth;
 $criteria['start_year'] = ($this->getSanParam('start-year')!="")?$this->getSanParam('start-year'):$startYear;
               // echo $startDay;
             
                
 $criteria['end_day'] = ($this->getSanParam('end-day')!="")?$report->check_length_add_one($this->getSanParam('end-day')):$endDay;
$criteria['end_month'] = ($this->getSanParam('end-month')!="")?$report->check_length_add_one($this->getSanParam('end-month')):$endMonth;
  $criteria['end_year'] = ($this->getSanParam('end-year')!="")?$this->getSanParam('end-year'):$endYear;
