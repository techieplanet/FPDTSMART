	...
	*/
	...
	public function facilityReport() {
		...
		if ($criteria ['go']) {
			...
			if ($criteria ['facility_sponsor_id'] or $criteria ['facility_sponsor_id'] === '0') {  			$where []= ' pt.sponsor_option_id = \'' . $criteria ['facility_sponsor_id'] . '\'';
			}
...
('num_location_tiers'));
		$this->viewAssignEscaped ( 'tlocations', $tlocations );
                ...
                $facility_location = explode("_",$facility);
                $facility_id = $facility_location[3];
...
$outdetails = array();
$hw_details = $db->fetchAll("SELECT first_name,middle_name,last_name,training_title_phrase,training_organizer_phrase FROM facility_summary_person_view WHERE facility_id='".$facility_id."'");
$title_array = array();
$title_value = array();
     ...
     //exit;
                  ...
                  $results = array();
                   ...
                   // $wise_word = "The length of the array is this ".$lent;
                    ...
                    else if($aggregate_method=="agrregate_data"){
                        $headers = array();
                        $output = array();
...
//                  
 ...
 $criteria['aggregate_method'] = $aggregate_method;
