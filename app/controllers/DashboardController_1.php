...
//	}
	...
	public function dash6aAction() {
        //public function trainedHWCoverageAction(){
        //TA:17:17: 01/16/2015
	    //Trained persons chart
...
//            }
            require_once('models/table/Dashboard-CHAI.php');
            $dashboardChai = new DashboardCHAI();
	    ...
	    if( !isset($_POST["region_c_id"] ) && !isset($_POST["district_id"] ) && !isset($_POST["province_id"] ) ) {        
                $tp_details = $dashboardChai->fetchTPDetails(3);
                $this->view->assign('tp_data', $tp_details);
	    }
	        ...
	        if( isset($_POST["region_c_id"]) && 
                ...
                { // CHAINigeria LGA
                    ...
                    $where .= 'AND flv.location_id IN (';
	            foreach ($_POST['region_c_id'] as $i => $value){
                        if($value == '') continue;
	                $geo = explode('_',$value);
	                $geoList = $geoList.$geo[2].', ';
	            }
                    
                    $geoList = substr(trim($geoList), 0, -1);  //remove trailing comma
	            $where .= $geoList.') ';
                    $groupFieldName = 'flv.location_id';
                    $havingName = 'f.location_id';
                    $tierValue = 3;
	            ...
	            $where .= 'AND flv.parent_id IN (';
	            foreach ($_POST['district_id'] as $i => $value){
                        if($value == '') continue;
	                $geo = explode('_',$value);
	                $geoList = $geoList.$geo[1].',';
	            }
	            
                    $geoList = substr(trim($geoList), 0, -1);  //remove trailing comma
	            $where .= $geoList.') ';
                    $groupFieldName = 'flv.parent_id';
                    $havingName = 'parent_id';
                    $tierValue = 2;
	            ...
	            foreach ($_POST['province_id'] as $i => $value){
                        if($value == '') continue;
	                $geo = explode('_',$value);
	                $geoList = $geoList.$geo[0].', ';
	            }
	            
                    $geoList = substr(trim($geoList), 0, -1);  //remove trailing comma
	            $where .= $geoList.') ';
                    $groupFieldName = 'flv.geo_parent_id';
                    $havingName = 'geo_parent_id';
                    $tierValue = 1;
	        } 
//                else { // no geo selection
//	            $group = 'l3.id';
...
//	        }
                ...
                $tp_details = $dashboardChai->fetchTPDetails(3, $where);
                $this->view->assign('tp_data', $tp_details);
                
                $ttc_details = $dashboardChai->fetchTTCDetails(3, $where, $groupFieldName, $havingName, $geoList, $tierValue);
                //var_dump($ttc_details); exit;
                $this->view->assign('ttc_fp_data', $ttc_details['fp_data']);
                $this->view->assign('ttc_larc_data', $ttc_details['larc_data']);
                $this->view->assign('ttc_locations', $ttc_details['location_names']);
                
            }//end else
            
            
            //GNR:use max commodity date
            $sDate = $dashboardChai->fetchTitleDate();
            $this->view->assign('tp_date', $sDate['month_name'] . ', '.$sDate['year']);
...
//	  } //else
	        ...
	        if( isset($_POST["region_c_id"]) && 
	            ...
	            //$location = 'National';
                    ...
                    $where .= 'AND flv.geo_parent_id IN (' . $geoList.') ';
                    //echo 'where: ' .$where; exit;
	        }
            ...
            $fp_coverage = $dashboardChai->fetchPercentFacHWTrainedDetails('fp', $locationNames, $where, $groupFieldName, $havingName, $geoList, $tierValue);
            $larc_coverage = $dashboardChai->fetchPercentFacHWTrainedDetails('larc', $locationNames, $where, $groupFieldName, $havingName, $geoList, $tierValue);
